...
impl<'a> AdafruitUltimateGps<'a> {
    ...
    pub async fn read_message(&mut self) -> String<96> {
        let mut string = Option::<String<96>>::None;

        loop {
            let len = self.usart.read(&mut self.buf).await.unwrap();

            for byte in self.buf.iter().take(len) {
                self.msg.push(*byte).unwrap();

                if *byte == b'\n' {
                    string = Option::Some(String::from(core::str::from_utf8(&self.msg).unwrap()));
